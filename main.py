...
> Size: XX GB
> Link: https://links.gamesdrive.net/#/link/{gdrive}.{uploader}
...
async def template(ctx, google_drive_link=None, actual_imdb_link=None, actual_size=None):
    if not google_drive_link:
        return await ctx.send(f"Please provide Google Drive Link. Run `{secrets.prefix}info template` for syntax.")
            ...
            async with ctx.typing():
                try:
                    ...
                    return await ctx.send(
                        f"Please provide imdb link as well. Run `{secrets.prefix}info template` for syntax.")
                try:
                    list_of_movies = imdb_obj.imdb_movie_list(
                        search_name_for_imdb)
                except:
                    return await ctx.send(
                        f"Please provide imdb link as well. Run `{secrets.prefix}info template` for syntax.")
                if len(list_of_movies) == 0:
                    try:
                        ...
                        if linkk:
                            mov_id = imdb_obj.id_from_imdblink(
                                imdb_obj.brute_imdb_link(search_name_for_imdb)).replace(" ", "")
                        else:
                            return await ctx.send(
                                f"Please provide imdb link as well. Run `{secrets.prefix}info template` for syntax.")
                        if mov_id == "":
                    ...
                    try:
                        ...
                        if msg.content.lower() == "none":
                            linkk = imdb_obj.brute_imdb_link(
                                search_name_for_imdb)
                            if linkk:
                                mov_id = imdb_obj.id_from_imdblink(
                                    imdb_obj.brute_imdb_link(search_name_for_imdb)).replace(" ", "")
                    ...
                    try:
                        try:
                            ...
                            if not msg.content.lower() == "none":
                                return await movie_list_msg.reply(
                                    "You have entered the index number which is not in this list")
                        imdb_id = imdb_obj.movie_id(final_movie)
                    except:
                        pass
            async with ctx.typing():
                list_of_genres = imdb_obj.get_genre_list(
                    imdb_obj.movie_obj(imdb_id))
...
async def info(ctx):
    em = discord.Embed(title="Megadrive Template Generator",
                       description="This bot will generate posting template for movies/series. You just need to input the gdrive link. Rest will be handled by the bot.\nHosted on Heroku.",
                       color=discord.Color.green())
    em.add_field(name="Made Bye",
                 value="jsmsj#5252\n(DMs Open for recommendations.)")
    em.add_field(name="Commands",
                 value=f"`{secrets.prefix}template`\n`{secrets.prefix}info template`\n`{secrets.prefix}info`")
    em.add_field(name="Repository Link",
                 value="[GitHub](https://github.com/jsmsj/template-generator-megadrive)")
    await ctx.send(embed=em)


@info.command()
async def template(ctx):
    em = discord.Embed(title="Info for `template` command",
                       description="Generate template for posting on megadrive using this command.",
                       color=discord.Color.random())
    em.add_field(
        name="Syntax", value=f"```fix\n{secrets.prefix}template <drive_link> [imdb_link] [size]\n```", inline=False)
    em.add_field(name="Examples",
    ...
    em.add_field(
        name="Note:", value="The size which is automatically detected by the bot does not happens for folders for now.",
        inline=False)
    em.set_footer(
        text="Parameters in <> are required, while in [] are optional")
    await ctx.send(embed=em)
